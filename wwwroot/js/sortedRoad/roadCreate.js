...
let roadVector = new ol.layer.Vector({
    source: new ol.source.Vector(),
    style: roadStyle,
    zIndex: 100
});

map.addLayer(roadVector);
...
let roadOrtoLength = document.getElementById("ortoLength");
...
let actionDraw = function (e) {
    ...
    roadVector.getSource().on('addfeature', function (evt) {
        if (roadVector.getSource().getState() === 'ready') {
            calculateLength(elementDOFLength, roadVector.getSource().getFeatures());
        }
    });
...
let actionDrawDelete = function (e) {
    ...
    let features = roadVector.getSource().getFeatures();
